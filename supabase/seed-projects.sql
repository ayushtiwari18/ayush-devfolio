...
(
  ...
  E'GameON is a full-stack tournament management system built on a MERN-inspired stack with Azure SQL Server as the database.\n\n- **Live scores** pushed via Socket.io WebSockets, eliminating manual refreshes.\n- **Role-based access** (Player / Organizer / Admin) enforced at the API layer using JWT + bcrypt.\n- **Azure SQL** provides relational guarantees needed for tournament brackets and match relationships.\n- **React Skeleton loaders** and Vite code-splitting cut initial render time by 50%.',

  E'Client (React + Vite)\n  -> REST API (Express MVC, Node.js)\n      -> Azure SQL Server (SSMS-managed schema)\n      -> Socket.io Server (real-time event bus)\n  -> JWT Middleware (role enforcement)\n  -> Netlify CDN (frontend) + Render (backend)\n\nStrict MVC pattern: routes/ -> controllers/ -> services/ -> models/. Each tournament entity (Match, Player, Team, Bracket) has a dedicated controller.',
  ...
  $$[
    ...
    {"title":"CI/CD via GitHub Actions","description":"Automated pipelines on Netlify and Render reduced integration bugs by 70% and enabled zero-downtime rollouts."}
  ]$$::jsonb,
  ...
  $$[
    ...
    {"problem":"JWT refresh strategy caused silent logouts on mobile","fix":"Implemented sliding-window token refresh with a 15-min access token and 7-day refresh token stored in httpOnly cookies."},
    {"problem":"React renders were expensive on the live leaderboard (100+ rows)","fix":"Used React.memo and virtualized list rendering to cap DOM nodes, cutting re-render time by 60%."}
  ]$$::jsonb,
  ...
  E'- Azure SQL over MongoDB: Relational integrity was worth the slightly higher query complexity for bracket management.\n- Socket.io over SSE: Chose bidirectional WebSockets because future features (live commentary, admin controls) need bidirectional channels.\n- Render for backend: Free tier cold-starts add ~2s on first hit. Acceptable for a portfolio/pilot project.',

  E'- Schema design first: Rushing the Azure SQL schema caused two costly migrations mid-project.\n- Room-based WebSockets: Learned the importance of namespace isolation in Socket.io early.\n- Team coordination at scale: Leading a 6-person team taught me the value of strict PR reviews, clear API contracts, and daily standups.',
...
(
  ...
  $$[
    ...
    {"title":"CI/CD with GitHub Actions","description":"Automated pipelines improved iteration speed 3x and cut rollback time from 30 minutes to under 5 minutes."}
  ]$$::jsonb,
...
(
  ...
  $$[
    ...
    {"title":"MongoDB Atlas for Scalability","description":"Cloud-hosted MongoDB ensured data availability during active sessions without managing a local database instance."}
  ]$$::jsonb,
  ...
  $$[
    ...
    {"problem":"Map markers did not cluster for dense listing areas","fix":"Implemented Mapbox GL supercluster plugin for automatic marker clustering, improving map readability for areas with 10+ listings."}
  ]$$::jsonb,
  ...
  E'- EJS over React: Deliberate choice to learn server-side rendering fundamentals.\n- Session auth over JWT: Stateful sessions are simpler for a classic MVC app but do not scale horizontally without a shared session store (solved with MongoDB session store).',
...
(
  ...
  $$[
    ...
    {"title":"React Custom Hooks","description":"Encapsulated WebRTC logic in useWebRTC.js and Socket.io logic in useSocket.js, keeping component code clean and testable."}
  ]$$::jsonb,
  ...
  $$[
    ...
    {"problem":"Socket.io room cleanup on abrupt disconnect left dangling participants","fix":"Added disconnect event handler to remove participant from room state and broadcast updated participant list to remaining peers."}
  ]$$::jsonb,
...
(
  ...
  E'After completing the Nullclass internship where I worked on a video streaming platform and reduced buffering time by 30%, I wanted to build my own YouTube-inspired frontend to solidify React patterns: API integration, SPA routing, search/filter UX, and optimized rendering for video-heavy content.',
  ...
  $$[
    ...
    {"title":"React Router DOM","description":"Client-side routing enables YouTube-style URL structure (/video/:id, /search/:query) without full page reloads."}
  ]$$::jsonb,
  ...
  $$[
    ...
    {"problem":"React re-renders on every search keystroke were expensive","fix":"Memoized VideoCard with React.memo and moved API call to useEffect with debounced input state."}
  ]$$::jsonb,
...
(
  ...
  $$[
    ...
    {"title":"Web Audio API for Tones","description":"Distinct sine-wave tones generated via AudioContext for each color - no audio file dependencies, works instantly across devices."}
  ]$$::jsonb,
  ...
  $$[
    ...
    {"problem":"Button flash duration was imperceptible on fast levels","fix":"Tied animation duration to a speed variable that decreases each level, keeping the flash visible but progressively shorter."}
  ]$$::jsonb,
...
(
  ...
  $$[
    ...
    {"title":"Semantic HTML","description":"Proper heading hierarchy, landmark elements, and alt text ensure accessibility and strong SEO baseline."}
  ]$$::jsonb,

  $$[
    {"problem":"Mobile layout broke on narrow viewports due to fixed-width project cards","fix":"Switched to CSS Grid with auto-fill minmax(280px, 1fr) for responsive card layout without media query overload."}
  ]$$::jsonb,
