...
-- Fix: all JSONB values use dollar-quoting ($$...$$) so
--      single quotes inside JSON strings never break parsing.
-- ============================================================
...
(
  ...
  $$[{"problem":"Unity WebGL bundles caused main thread blocking on load, degrading Lighthouse performance score","fix":"Moved Unity builds to async iframe embeds with IntersectionObserver-based lazy load - only loads when the lab section enters viewport."},
    ...
    {"problem":"Multilingual content structure was not designed in the initial schema","fix":"Refactored content documents to use a locale map object (en, hi, mr keys) enabling non-breaking language addition."}]$$::jsonb,
