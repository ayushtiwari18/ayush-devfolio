...
export default function NewEventPage() {
  ...
  const [form, setForm] = useState({
    ...
    images: '',          // comma-separated URLs -> array on save
    technologies: '',    // comma-separated -> array on save
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl">
      ...
      <div className={SECTION}>
        ...
        <div>
          ...
          <input className={INPUT} value={form.slug} onChange={e => set('slug', e.target.value)} placeholder="smart-india-hackathon-2024" required />
      ...
      <div className={SECTION}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input className={INPUT} value={form.location} onChange={e => set('location', e.target.value)} placeholder="New Delhi, India" />
          </div>
          <div>
            <label className={LABEL}>Duration</label>
            <input className={INPUT} value={form.duration} onChange={e => set('duration', e.target.value)} placeholder="36 hours" />
          </div>
        </div>
        <div>
          <label className={LABEL}>Organizer</label>
          <input className={INPUT} value={form.organizer} onChange={e => set('organizer', e.target.value)} placeholder="Ministry of Education, Govt. of India" />
        </div>
      </div>
      ...
      <div className={SECTION}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" min="1" className={INPUT} value={form.team_size} onChange={e => set('team_size', e.target.value)} />
          </div>
          <div>
            <label className={LABEL}>Team Name</label>
            <input className={INPUT} value={form.team_name} onChange={e => set('team_name', e.target.value)} placeholder="Team Nexus" />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className={LABEL}>Your Role</label>
            <input className={INPUT} value={form.role} onChange={e => set('role', e.target.value)} placeholder="Team Lead & Backend Developer" />
          </div>
          <div>
            <label className={LABEL}>Result</label>
            <input className={INPUT} value={form.result} onChange={e => set('result', e.target.value)} placeholder="Winner 🏆 / Finalist / Participant" />
          </div>
        </div>
        <div>
          <label className={LABEL}>Prize</label>
          <input className={INPUT} value={form.prize} onChange={e => set('prize', e.target.value)} placeholder="₹1,00,000 cash prize" />
        </div>
      </div>
      ...
      <div className={SECTION}>
        ...
        <div>
          ...
          <textarea rows={2} className={INPUT} value={form.description} onChange={e => set('description', e.target.value)} placeholder="1–2 sentence summary for the listing card" />
        </div>
        <div>
          <label className={LABEL}>Story <span className="text-muted-foreground font-normal">(full narrative on detail page)</span></label>
          <textarea rows={8} className={INPUT} value={form.story} onChange={e => set('story', e.target.value)} placeholder="Tell the full story\u2014what you built, how, obstacles, what you learned...\n\nUse blank lines to separate paragraphs." />
          <p className={HINT}>Plain text. Separate paragraphs with a blank line.</p>
        </div>
        <div>
          <label className={LABEL}>Technologies <span className="text-muted-foreground font-normal">(comma-separated)</span></label>
          <input className={INPUT} value={form.technologies} onChange={e => set('technologies', e.target.value)} placeholder="React, Node.js, MongoDB, Socket.io" />
        </div>
      </div>
      ...
      <div className={SECTION}>
        ...
        <div>
          <label className={LABEL}>Gallery Images <span className="text-muted-foreground font-normal">(comma-separated URLs)</span></label>
          <textarea rows={2} className={INPUT} value={form.images} onChange={e => set('images', e.target.value)} placeholder="https://url1.jpg, https://url2.jpg, https://url3.jpg" />
        </div>
        <div>
          <label className={LABEL}>Certificate Image URL</label>
          <input className={INPUT} value={form.certificate_image} onChange={e => set('certificate_image', e.target.value)} placeholder="https://..." />
        </div>
      </div>
      ...
      <div className={SECTION}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input className={INPUT} value={form.links_github} onChange={e => set('links_github', e.target.value)} placeholder="https://github.com/..." />
          </div>
          <div>
            <label className={LABEL}>Devpost</label>
            <input className={INPUT} value={form.links_devpost} onChange={e => set('links_devpost', e.target.value)} placeholder="https://devpost.com/..." />
          </div>
          <div>
            <label className={LABEL}>Certificate URL</label>
            <input className={INPUT} value={form.links_certificate} onChange={e => set('links_certificate', e.target.value)} placeholder="https://..." />
          </div>
          <div>
            <label className={LABEL}>Live Demo</label>
            <input className={INPUT} value={form.links_live} onChange={e => set('links_live', e.target.value)} placeholder="https://..." />
          </div>
          <div>
            <label className={LABEL}>Article / Blog</label>
            <input className={INPUT} value={form.links_article} onChange={e => set('links_article', e.target.value)} placeholder="https://..." />
          </div>
        </div>
      </div>
