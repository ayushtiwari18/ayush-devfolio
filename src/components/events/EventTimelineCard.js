...
// ── Main card ──────────────────────────────────────────
export default function EventTimelineCard({ event, index = 0 }) {
  const cfg         = TYPE_CONFIG[event.type] || TYPE_CONFIG.other;
  const imgs        = Array.isArray(event.images) ? event.images : [];
  const { ref, visible } = useReveal({ threshold: 0.1 });
  const delay       = Math.min(index * 0.08, 0.32); // stagger capped at 320ms

  return (
    <div ref={ref} style={fadeUp(visible, delay)}>
